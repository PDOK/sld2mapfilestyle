...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="outputStroke">
		...
		<xsl:call-template name="outputElemValueWithQuotes">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke']"/>
			<xsl:with-param name="prefix" select="concat('        ', $outlinePrefix, 'COLOR ')"/>
		</xsl:call-template>
		<xsl:call-template name="outputElemValue">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-width']"/>
		...
		<xsl:call-template name="outputOpacity">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-opacity']"/>
			<xsl:with-param name="prefix" select="'        OPACITY '"/>
		</xsl:call-template>
		<xsl:call-template name="outputElemValue">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-linecap']"/>
			<xsl:with-param name="prefix" select="'        LINECAP '"/>
		</xsl:call-template>
		<xsl:call-template name="outputLineJoin">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-linejoin']"/>
			<xsl:with-param name="prefix" select="'        LINEJOIN '"/>
		</xsl:call-template>
		<xsl:call-template name="outputElemValue">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-dasharray']"/>
			<xsl:with-param name="prefix" select="'        PATTERN '"/>
			<xsl:with-param name="suffix" select="' END'"/>
		</xsl:call-template>
		<xsl:call-template name="outputElemValue">
			<xsl:with-param name="elem" select="se:Stroke/se:SvgParameter[@name='stroke-dashoffset']"/>
	...
	<xsl:template name="outputFill">
		<xsl:call-template name="outputElemValueWithQuotes">
			<xsl:with-param name="elem" select="se:Fill/se:SvgParameter[@name='fill']"/>
			<xsl:with-param name="prefix" select="'        COLOR '"/>
		</xsl:call-template>
		<xsl:call-template name="outputOpacity">
			<xsl:with-param name="elem" select="se:Fill/se:SvgParameter[@name='fill-opacity']"/>
			<xsl:with-param name="prefix" select="'        OPACITY '"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="outputClasses">
		...
		<xsl:if test="$rules">
			...
			<xsl:for-each select="$rules">
				...
				<xsl:for-each select="se:LineSymbolizer">
					<xsl:text>      STYLE&#xa;</xsl:text>
				...
				<xsl:for-each select="se:PolygonSymbolizer">
					<xsl:text>      STYLE&#xa;</xsl:text>
				...
				<xsl:for-each select="se:PointSymbolizer">
				 	<xsl:variable name="increment" select="position()"/>
					<xsl:for-each select="se:Graphic/se:Mark">
						<xsl:text>      STYLE&#xa;</xsl:text>
						
						<!-- Symbol properties -->
						<!-- The content of this symbol is being exported by sld2namedStyles.xslt -->
						<xsl:call-template name="outputElemValue">
							<xsl:with-param name="elem" select="../../../se:Name"/>
							<xsl:with-param name="prefix" select="'        SYMBOL &quot;'"/>
							<xsl:with-param name="suffix" select="concat('-', $increment, '&quot;')"/>
						</xsl:call-template>
						<xsl:call-template name="outputElemValue">
							<xsl:with-param name="elem" select="../se:Size"/>
		...
		<xsl:if test="//ogc:Filter/ogc:PropertyIsEqualTo/ogc:Literal">
			...
			<xsl:for-each select="//ogc:Filter/ogc:PropertyIsEqualTo/ogc:Literal/text()[generate-id() = generate-id(key('filterValue',.)[1])]">
				...
				<xsl:call-template name="outputClasses">
					...
					<xsl:with-param name="rules" select="//se:Rule[ogc:Filter/ogc:PropertyIsEqualTo/ogc:Literal/text()=$filterValue]"/>
				</xsl:call-template>
			</xsl:for-each>
		</xsl:if>
